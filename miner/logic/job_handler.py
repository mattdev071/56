...
def _create_docker_entrypoint(job, gpu_ids: List[int] = None):
    ...
    # Optimize accelerate launch for multi-GPU
...
def start_tuning_container(job: TextJob, gpu_ids: List[int] = None):
    ...
    # Optimize config for multi-GPU training
    if gpu_ids and len(gpu_ids) > 1:
        config = _optimize_config_for_multi_gpu(config, len(gpu_ids), job.model)
    
    save_config(config, config_path)
    logger.info(f"Training config optimized for {len(gpu_ids) if gpu_ids else 1} GPUs")
    ...
    # Add multi-GPU environment variables
    if gpu_ids:
    ...
    try:
        ...
        # Configure device requests for multi-GPU
        device_requests = []
        if gpu_ids:
        ...
        container = docker_client.containers.run(
            ...
            detach=True,
            tty=True,
            shm_size="32g",  # Increased for multi-GPU
...
def _optimize_config_for_multi_gpu(config: dict, num_gpus: int, model_name: str) -> dict:
    """Optimize training configuration for multi-GPU training"""
    
    # Estimate model size
    model_size = _estimate_model_size_from_name(model_name)
    
    # Adjust batch sizes and accumulation steps
    if num_gpus >= 4:
        config["micro_batch_size"] = max(1, config.get("micro_batch_size", 2) * 2)
        config["gradient_accumulation_steps"] = max(1, config.get("gradient_accumulation_steps", 4) // 2)
    elif num_gpus >= 2:
        config["micro_batch_size"] = max(1, config.get("micro_batch_size", 2) * 1.5)
        config["gradient_accumulation_steps"] = max(1, config.get("gradient_accumulation_steps", 4) // 1.5)
    
    # Optimize LoRA settings for larger models
    if model_size > 35:
        config["lora_r"] = min(256, config.get("lora_r", 8) * 2)
        config["lora_alpha"] = min(512, config.get("lora_alpha", 16) * 2)
    elif model_size > 13:
        config["lora_r"] = min(128, config.get("lora_r", 8) * 1.5)
        config["lora_alpha"] = min(256, config.get("lora_alpha", 16) * 1.5)
